...
import { dedupeColorEntries, dedupeColors, NO_SIZE_PARAM, normalizeColor, sortSizes } from '@/lib/utils'
...
import { withCoverFirst, withCoverFirstAll } from '@/lib/coverMedia'
...
const fetchFeaturedProducts = async () => {
    ...
    // Attach a default variant per product so storefront cards can quick-add to
    // the cart, which keys on variantId. There is no pricing to rank by, so the
    // first-created live variant is the stable choice.
    ...
    return toPlainObject(withCoverFirstAll(enriched))
}
...
const fetchFreshlyArrivedProducts = async () => {
    ...
    const selectFields = 'name slug media coverMedia freshlyArrivedSortOrder'
    ...
    return toPlainObject(withCoverFirstAll(products.slice(0, FRESHLY_ARRIVED_COUNT)))
}
...
const fetchProductDetailsBySlug = async (slug, size, color) => {
    ...
    // `?size=__none__` explicitly selects the sizeless variant; any other
    // non-empty value is an exact match. Plain empty means "caller does not
    // care", which leaves size unconstrained.
    if (size === NO_SIZE_PARAM) variantFilter.size = { $in: ['', null] }
    else if (size) variantFilter.size = size
    ...
    // Fall back in decreasing specificity rather than straight to "any variant".
    // Size is optional, so a stale or hand-edited `?size=` is common - dropping
    // colour along with it used to land the shopper on a completely different
    // colour than the swatch they clicked. Try colour-only first, and only then
    // give up and take the first live variant.
    if (!variant && variantFilter.color) {
        variant = await ProductVariantModel.findOne({
            product: product._id,
            deletedAt: null,
            color: variantFilter.color,
        })
            .sort({ _id: 1 })
            .populate('media', 'secure_url alt')
            .lean()
    ...
    // Sizes in first-seen order, then sorted. Size is optional, so blanks are
    // excluded here - the client surfaces the sizeless variants as their own
    // "One size" option instead of as an empty pill.
    const sizeSeen = []
    ...
    // size is coerced to '' so rows written before it had a default (undefined)
    // key the same way as a deliberately sizeless one - the client builds a
    // `color|size` lookup key from these and `undefined` would never match.
    ...
    return toPlainObject({
        product: withCoverFirst(product),
        variant: withCoverFirst(variant),
