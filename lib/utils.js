...
// encoded - and legacy rows can be DOUBLE- or TRIPLE-encoded (e.g.
// `&amp;amp;lt;p&amp;amp;gt;`), which renders visible literal tags like
// "<p>&nbsp;Why it's called…" on the storefront. A single `decode()` only peels
// one layer, so this peels until the string stabilises.
//
// The stop condition must test for *any* remaining entity, not just `&lt;` /
// `&gt;`: peeling `&amp;amp;lt;` yields the intermediate `&amp;lt;`, which
// contains no literal `&lt;` substring, so a `&lt;|&gt;` guard bailed out one
// pass early and left the entities on screen.
export const decodeHTMLDeep = (value, maxPasses = 5) => {
  ...
  for (let i = 0; i < maxPasses; i++) {
    ...
    current = next;
    if (!/&(?:[a-z][a-z0-9]*|#[0-9]+|#x[0-9a-f]+);/i.test(current)) break;
  }
  return current;
};
...
// URL token meaning "the sizeless variant", used by ?size= on the product page.
// A product can hold both a sizeless and a sized variant of the same colour, so
// a bare ?color=green is ambiguous - findOne would return whichever Mongo
// yields first. Deliberately not a plausible admin-typed size.
export const NO_SIZE_PARAM = '__none__'

// Sizes are free-form and admin-authored ("6 inch", "3 ft", "L"), so no single
// rule orders them all. Lettered apparel sizes keep their canonical scale,
// anything numeric sorts numerically ("6 inch" before "10 inch", which alpha
// sort would invert), and the rest fall back to locale compare so the order is
// at least deterministic. Blank sizes never reach here - callers filter them.
