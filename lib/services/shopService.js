...
export const getShopProducts = async (params = {}) => {
    ...
    // $lookup does not preserve the order of the `media` id array, so the shop
    // grid's first image was effectively arbitrary. Rotating the admin-chosen
    // cover to the front fixes both that and the cover feature in one step.
