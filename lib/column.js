...
export const DT_PRODUCT_VARIANT_COLUMN = [
    ...
    // Joined from the variant's product (parent lives on its category). Present
    // so the toolbar's parent/category dropdowns have columns to drive - hide
    // them from the View menu if the table feels wide.
    {
        accessorKey: 'parent',
        header: 'Parent',
    },
...
export const DT_ENQUIRY_COLUMN = [
  ...
  },
  {
    ...
    header: 'Items / Qty',
    Cell: ({ row }) => {
      const items = row?.original?.totalItem ?? row?.original?.products?.length ?? 0
      const totalQty = row?.original?.totalQty ?? row?.original?.products?.reduce((sum, p) => sum + (p.qty || 0), 0) ?? items
      return (
        <span className="font-medium whitespace-nowrap">
          {items} {items === 1 ? 'item' : 'items'}
          <span className="text-muted-foreground text-xs ml-1">({totalQty} {totalQty === 1 ? 'unit' : 'units'})</span>
        </span>
      )
    },
  },
