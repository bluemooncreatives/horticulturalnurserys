...
export const zSchema = z.object({
    ...
    parent: z.string().min(3, 'Parent is required.'),

    description: z.string().min(3, 'Description is required.'),
    media: z.array(z.string()),
    // Optional: the id of the media entry the admin picked as the cover. The
    // write routes drop it when it is not among the submitted media, so a stale
    // id can never be persisted.
    coverMedia: z.string().optional().or(z.literal('')).or(z.null()),
    ...
    colorHex: z
        ...
        .optional()
        .or(z.literal('')),
    // Optional - a variant without a size is valid (see ProductVariant model).
    // preprocess keeps the output a plain string so every consumer (create and
    // update routes, both admin forms) gets '' rather than undefined.
    size: z.preprocess(
        (value) => (typeof value === 'string' ? value.trim() : ''),
        z.string().max(60, 'Size is too long.')
    ),
    ...
    // Display name for an admin-authored review (there is no customer account).
    authorName: z.string().trim().min(2, 'Reviewer name is required.').max(60, 'Reviewer name is too long.'),
